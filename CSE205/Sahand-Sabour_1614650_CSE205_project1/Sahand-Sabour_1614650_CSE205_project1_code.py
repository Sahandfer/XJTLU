...
print('Please enter the URL: (starting with http:// or https://)')
userUrl= input().strip()
...
userDepth= input("Please input the desired depth (press enter to skip): ")
if(userDepth==''):
    userDepth=5
else:
    userDepth= int(userDepth)
...
def downloadImages(current_host, current_sub, image):
...
for depth in range(1, userDepth+2):
    ...
    for prev_href in prev_hrefs:
        ...
        # Making threads
        threads=[]
        prev_href_new= prev_href
        if (prev_href[-1] != '/'):
            prev_href_new = prev_href+'/'
        imageList = findImages(prev_href_new)
        for image in imageList:
            current_host, current_sub = separator(prev_href_new)
            thread = threading.Thread(target=downloadImages, args=(current_host, current_sub, image))
            # Now we have a list of threads for each of the images in the image list
            threads.append(thread)

        # Running the threads in the list
        for t in threads:
            t.start()
        for t in threads:
            t.join()
